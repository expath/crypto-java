...
public class GenerateAWSsignature extends BaseTest {
	...
	public void hmacStringWithSha256() throws Exception {
		...
		byte[] kDate = Hmac.hmac(dateStamp.getBytes(StandardCharsets.UTF_8), kSecret.getBytes(StandardCharsets.UTF_8),
				"HMAC-SHA-256", "base64");
		String kDateHexValue = DatatypeConverter.printHexBinary(Hmac.hmac(dateStamp.getBytes(StandardCharsets.UTF_8),
				kSecret.getBytes(StandardCharsets.UTF_8), "HMAC-SHA-256", "hex")).toLowerCase();
		Assert.assertTrue(kDateHexValue.equals("969fbb94feb542b71ede6f87fe4d5fa29c789342b0f407474670f0c2489e0a0d"));

		byte[] kRegion = Hmac.hmac(regionName.getBytes(StandardCharsets.UTF_8), kDate, "HMAC-SHA-256", "base64");
		String kRegionHexValue = DatatypeConverter
				.printHexBinary(Hmac.hmac(regionName.getBytes(StandardCharsets.UTF_8), kDate, "HMAC-SHA-256", "hex"))
				.toLowerCase();
		Assert.assertTrue(kRegionHexValue.equals("69daa0209cd9c5ff5c8ced464a696fd4252e981430b10e3d3fd8e2f197d7a70c"));

		byte[] kService = Hmac.hmac(serviceName.getBytes(StandardCharsets.UTF_8), kRegion, "HMAC-SHA-256", "base64");
		String kServiceHexValue = DatatypeConverter
				.printHexBinary(Hmac.hmac(serviceName.getBytes(StandardCharsets.UTF_8), kRegion, "HMAC-SHA-256", "hex"))
				.toLowerCase();
		...
		byte[] kSigning = Hmac.hmac("aws4_request".getBytes(StandardCharsets.UTF_8), kService, "HMAC-SHA-256",
				"base64");
		String kSigningHexValue = DatatypeConverter
				.printHexBinary(
						Hmac.hmac("aws4_request".getBytes(StandardCharsets.UTF_8), kService, "HMAC-SHA-256", "hex"))
				.toLowerCase();
